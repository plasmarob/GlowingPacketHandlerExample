 ...
 */
public class GlowPacketHandler extends ChannelDuplexHandler {
	private Player p;
	private Entity e;

	public GlowPacketHandler(final Player p, final Entity e) {
		this.p = p;
		this.e = e;
	}
	/*
		...
		if(msg.getClass().getSimpleName().equalsIgnoreCase("PacketPlayOutEntityMetadata")) {
			...
			if ( ((CraftEntity)e).getEntityId() == eid) {
				...
				if (type.equals("Byte")) {
					...
					//glow is 0x40 part of the byte, replace it in with binary OR
					databyte = (byte)(databyte | 0x40); 
